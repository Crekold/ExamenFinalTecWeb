...
export const insertCliente = (data, result) => {
    db.query("INSERT INTO pacientes ( ci, nombre, sangre) VALUES ( ?, ?, ?)", [data.ci, data.nombre, data.sangre], (err, results) => {             
        if(err) {
            console.log(err);
            result(err, null);
        } else {
...
export const deleteClienteById = (id, result) => {
    db.query("DELETE FROM pacientes WHERE id_cliente = ?", [id], (err, results) => {             
        if(err) {
            console.log(err);
            result(err, null);
        } else {
